...
<Note>
  Trails is responsible for orchestrating the flow, not holding user assets. It coordinates trusted protocols so users can transact with a single click (one transaction on the original chain), while assets move through underlying networks.
 </Note>


## Security & Validation

- **Built on Sequence v3**: Trails is built on top of Sequence's patented v3 wallet contracts, leveraging mature account abstraction and battle-tested controls.
...
- **User “sudo” Control**: The user always retains superuser (sudo) control over their account and funds. In emergencies, they can take direct action at any time.
