...
description: 'Core architecture and design principles of the Trails for cross-chain transactions'
icon: 'sitemap'
---

Trails's Trails simplifies cross-chain transactions through an intelligent 5-step process that handles the complexity of multi-chain operations behind the scenes.

## Overview

The Trails protocol automatically determines the optimal path for cross-chain payments, whether it's a simple single-chain transaction or a complex multi-step orchestration involving swaps, bridges, and executions across different blockchains.
...
<Steps>
  ...
  <Step title="Trails Inspects">
    **Determines if orchestration is needed**
    ...
    Trails's intelligent inspection engine analyzes the defined intent to determine the optimal execution path:
