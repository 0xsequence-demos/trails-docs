...
<Warning>
**Deprecated for new integrations.** As of `0xtrails@0.18.0`, the SDK skips `CommitIntent` and calls [`ExecuteIntent`](/api-reference/endpoints/execute-intent) directly on a `QUOTED` intent (or [`ExecuteIntentEdge`](/api-reference/endpoints/execute-intent-edge) for origin-mode edge intents). The endpoint stays behaviorally stable for existing integrations that still call it.
</Warning>
